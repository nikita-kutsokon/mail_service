...
const createNewMailTemplateFile = async (templateName, templateFile) => {
    ...
    const createdFile = await config_1.default.files.create({
        requestBody: {
            name: templateName,
            mimeType: 'text/html',
...
const getMailTemplateFileDataById = async (fileId) => {
    const response = await config_1.default.files.get({ fileId: fileId, alt: 'media' }, { responseType: 'stream' });
...
const updateMailTemplateFileById = async (fileId, data) => {
    const updatedFile = await config_1.default.files.update({
        fileId: fileId,
        media: {
            mimeType: 'text/html',
            body: data
...
const deleteMailTemplateFileById = async (fileId) => {
    const deletedFile = await config_1.default.files.delete({ fileId });
    return deletedFile;
};
exports.default = {
